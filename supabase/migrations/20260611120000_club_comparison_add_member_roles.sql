-- Add member_roles to each team in get_club_comparison_dashboard.
-- Roster counts cover all role grades (players + management) and are
-- independent of the event date range — they reflect the current team
...
BEGIN
    ...
    team_members AS (
        -- One row per (team, member) for event attendance; grade 10 only
    ...
    -- Roster: all role grades, one primary role per member per team
    team_roster AS (
        SELECT DISTINCT ON (mtl.team_id, m.member_id)
            mtl.team_id,
            m.member_id,
