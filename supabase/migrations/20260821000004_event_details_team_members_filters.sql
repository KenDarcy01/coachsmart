-- Tighten the team_members list in get_user_event_details with three filters:
--   1. Exclude members whose ONLY roles are admin (role_grade = 100).
--      Adding r.role_grade = 10 means only player-grade role rows are
--      considered; a member with both admin and player roles still appears.
--   2. Exclude members not in the event's squad (when squad_id is set),
--      using the same member_squad_link + v_effective_code_id check already
--      applied in categorized_eligible_members.
--   3. Exclude members whose role level is below the event's required
--      role level (r.role_level >= ed.event_role_level). When event_role_level
--      is 0 (no restriction) every grade-10 member qualifies.
