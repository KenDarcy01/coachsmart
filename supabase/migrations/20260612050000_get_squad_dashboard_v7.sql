-- Squad dashboard v7: drop latest_squad_saves, count distinct events per member
--
-- v6 bug: latest_squad_saves picked the globally highest match_squad_id for
--   each (event_id, squad_id) pair across all members.  If a squad was
--   re-saved after the fact and a player was accidentally omitted from that
--   later save, the player would be excluded from match_sels even though they
--   genuinely appeared in an earlier (correct) save.
--
--   Example: event 633 (Feb 14), Blue Team saves 259 and 260 both include
--   Alex Beggs, but a subsequent save (id > 260) re-saved the squad without
--   him.  v6 picked the later save → Alex missed → table showed 9 instead
--   of 10.  The drawer (which shows any save) correctly showed 10.
--
-- v7 fix: remove latest_squad_saves entirely.  Join match_squad_details
--   directly to filtered_events and use COUNT(DISTINCT event_id) per member.
--   DISTINCT handles all revision deduplication: a player with 7 revision rows
--   for the same event still counts as 1 appearance.  This matches exactly
--   what the drawer computes.
--
-- Performance: add an index on match_squad_details(event_id) to support the
--   direct join to filtered_events.

CREATE INDEX IF NOT EXISTS idx_match_squad_details_event_id
    ON public.match_squad_details (event_id);
...
BEGIN
    ...
    -- Accepted attendee count per event for the attendance threshold filter.
    event_att_counts AS (
        SELECT
            e.event_id,
    ...
    -- Events that pass all date/code/type/attendance filters.
    filtered_events AS (
        SELECT e.event_id
        FROM public.events e
        JOIN club_teams ct               ON e.team_id        = ct.team_id
    ...
    -- Count distinct events per (member, team, squad) across any save.
    -- COUNT(DISTINCT event_id) deduplicates revision saves automatically:
    -- a player with multiple save rows for the same event counts as 1.
    -- A player who appeared in ANY save for an event is counted, even if a
    -- later accidental re-save omitted them — matching drawer behaviour.
    match_sels AS (
        SELECT
            msd.member_id,
            msd.team_id,
    ...
    -- Members sourced from match_sels (actual selections, not roster).
    -- Blank names and members not in team_member_roles are excluded via FILTER.
