-- Exclude the member who made the attendance change from receiving their own notification.
-- Previously, admin users who changed their own attendance would receive a notification
-- about the change they just made. Fixed by adding m.member_id != p_member_id_param
-- to the admin_users CTE.
...
BEGIN
    ...
    admin_users AS (
        ...
        WHERE mtrl.role_id = 7
          AND m.member_id != p_member_id_param  -- exclude the member who made the change
    )
    SELECT
        au.user_id,
        te.team_id,
