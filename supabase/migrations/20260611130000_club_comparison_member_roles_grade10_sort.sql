-- Update get_club_comparison_dashboard: member_roles now filters to
-- role_grade = 10 only and sorts pills by role_level ascending.
...
BEGIN
    ...
    -- Roster: grade 10 only, one primary role per member, sorted by role_level
    team_roster AS (
        SELECT DISTINCT ON (mtl.team_id, m.member_id)
            mtl.team_id,
            m.member_id,
