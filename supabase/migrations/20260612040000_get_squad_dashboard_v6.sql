-- Squad dashboard v6: fix multi-squad-per-event under-counting
--
-- v5 bug: latest_match_squads used DISTINCT ON (event_id), picking only ONE
--   match_squads row per event (the highest match_squad_id).  FlutterFlow
--   creates a separate match_squads row for each squad colour (Blue, Red,
--   Green …) within the same save.  Members in any squad other than the one
--   with the highest match_squad_id were silently excluded from match_sels,
--   causing match_count to be under-reported in the table.
--
-- v6 fix: bypass match_squads entirely.  Deduplicate directly in
--   match_squad_details on (event_id, squad_id), keeping the row with the
--   highest match_squad_id for each (event, squad) pair.  This correctly
--   handles multiple squads per event and multiple saves (revisions) per squad.
...
BEGIN
    ...
    -- Step 2: for each (event_id, squad_id) pair keep only the detail row
    -- with the highest match_squad_id (= most recent save of that squad for
    -- that event).  This correctly handles:
    --   a) multiple squad colours per event (Blue / Red / Green all kept), and
    --   b) multiple revision saves of the same squad (only latest kept).
    latest_squad_saves AS (
        SELECT DISTINCT ON (msd.event_id, msd.squad_id)
            msd.event_id,
            msd.squad_id,
            msd.match_squad_id
        FROM public.match_squad_details msd
        JOIN filtered_events fe ON msd.event_id = fe.event_id
        ORDER BY msd.event_id, msd.squad_id, msd.match_squad_id DESC
    ),
    -- Step 3: count distinct event appearances per (member, team, squad).
    -- Join back to match_squad_details using all three keys so only rows that
    -- belong to the latest save of each squad are counted.
    match_sels AS (
        SELECT
            msd.member_id,
            msd.team_id,
