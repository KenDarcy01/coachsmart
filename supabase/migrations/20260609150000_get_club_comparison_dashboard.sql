-- Club-scoped comparison dashboard RPC.
-- Returns all teams for a club in a date range, each with per-event
-- data and per-member responses embedded for client-side filtering.
...
BEGIN
    ...
    team_members AS (
        -- One row per (team, member); DISTINCT ON picks primary role via role_list_seq
    ...
    team_event_rows AS (
        -- One row per (team, event) with members array
        SELECT
            e.team_id,
            e.event_id,
            e.event_date_time,
