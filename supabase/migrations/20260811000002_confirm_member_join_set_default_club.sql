-- When a membership is approved, populate users.default_club from the
-- approved team's club_id. Only set if default_club is NULL so that
-- the first team joined wins and subsequent approvals don't overwrite it.
...
BEGIN
    ...
    -- Mark the user as fully onboarded and set their default club
    -- (default_club only set if NULL — first approved team wins)
    UPDATE public.users
