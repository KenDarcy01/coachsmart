  ...
  IF has_pending_notifications THEN
    PERFORM net.http_post(
      ...
      headers := jsonb_build_object(
        ...
        'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imd5ZnBvcnNiZGZ0dnRha2R2dWt0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTczMzMyOTEyMCwiZXhwIjoyMDQ4OTA1MTIwfQ.7ayxwSgZyGZsGPkstBLPR145qlEu3cmEYFOuHzyzGgg'
      ),
      body := '{}'::jsonb
    );
  END IF;
...
BEGIN
    ...
    IF v_event_code_id IS NOT NULL THEN
        ...
        IF NOT v_has_members_for_code THEN
            ...
            WHERE team_id = v_team_id
              AND code_id IS NOT NULL
              AND code_id > v_event_code_id  -- Look for codes greater than the event's code
            ORDER BY code_id ASC  -- Get the next smallest code_id
            LIMIT 1;
            
            -- If found a next code, use it
            IF v_next_code_id IS NOT NULL THEN
                v_effective_code_id := v_next_code_id;
            ELSE
                -- If no higher codes, try to find any code for this team (lowest available)
                SELECT MIN(code_id) INTO v_effective_code_id
                FROM public.member_squad_link
                WHERE team_id = v_team_id
                  AND code_id IS NOT NULL;
        ...
        END IF;
    ELSE
        -- If event has no code_id, find the lowest code_id available for the team
        SELECT MIN(code_id) INTO v_effective_code_id
        FROM public.member_squad_link
        WHERE team_id = v_team_id
          AND code_id IS NOT NULL;
    ...
    -- If still no code found, try to find any code from club_code_link for this team's club
    IF v_effective_code_id IS NULL THEN
        SELECT ccl.code_id INTO v_effective_code_id
        FROM public.teams t
        JOIN public.clubs c ON t.club_id = c.club_id
        JOIN public.club_code_link ccl ON c.club_id = ccl.club_id
        WHERE t.team_id = v_team_id
        ORDER BY ccl.code_id ASC  -- Get the smallest code_id
        LIMIT 1;
    END IF;
    ...
    actual_member_squad_roles_for_event AS (
        -- CTE 2: Finds all distinct Member/Squad/Role combinations using member_squad_link with the effective code_id
        SELECT DISTINCT
            e.event_id,
            msl.member_id,
            msl.squad_id,
        ...
        FROM
            events e
            -- Join member_squad_link using the effective code_id (either event's code or fallback)
            JOIN public.member_squad_link msl ON e.team_id = msl.team_id 
                AND msl.code_id = v_effective_code_id  -- Use the effective code_id
            -- Join through member_team_link to get to member_team_role_link
            JOIN public.member_team_link mtl ON msl.member_id = mtl.member_id 
                AND msl.team_id = mtl.team_id
            JOIN public.member_team_role_link mtrl ON mtl.member_team_id = mtrl.member_team_id
        WHERE
    ...
    all_squad_roles_for_event AS (
        -- CTE 3: Generates the base set of all Squads and Roles that should be reported on, including filtering
        SELECT
            e.event_id,
            sq.squad_id,
            sq.squad_name,
    ...
    member_status_data AS (
        -- CTE 4: Links member details and attendance status
        SELECT
            amsr.event_id,
            amsr.squad_id,
            amsr.role_id,
    ...
    squad_dynamic_role_count AS (
        -- CTE 5: Calculates the member count based on p_role_level_filter and p_response_id.
        SELECT
            msd.squad_id,
        ...
        WHERE
            -- If p_role_level_filter is provided, filter by it
            (p_role_level_filter IS NULL OR p_role_level_filter = 0 OR r.role_level = p_role_level_filter)
            -- Response filter
            AND (
                -- If p_response_id is 3 (Accepted) or 4 (Declined), match the response ID directly
                (p_response_id IN (3, 4) AND msd.response_id = p_response_id)
                -- If p_response_id is NULL or other, count members with no response (IS NULL)
    ...
    roles_with_counts AS (
        -- CTE 6: Aggregates the three response counts
        SELECT
            r.event_id,
            r.squad_id,
            r.squad_name,
    ...
    squads_with_roles AS (
        -- CTE 7: Aggregates Role objects into Squad objects
        SELECT
            rwc.event_id,
            rwc.squad_id,
            rwc.squad_name,
    ...
    -- Final SELECT: Aggregates everything into the final top-level JSONB object with fallback info
    SELECT
        jsonb_build_object(
            'event_id', sq.event_id,
            'team_name', (SELECT t.team_name FROM public.events e JOIN public.teams t ON e.team_id = t.team_id WHERE e.event_id = p_event_id LIMIT 1),
...
CREATE OR REPLACE FUNCTION "public"."get_members_attendance_for_event"("p_event_id" bigint, "p_user_id" "uuid") RETURNS TABLE("member_id" bigint, "first_name" "text", "last_name" "text", "profile_pic" "text", "response_value" "text", "response_icon" "text", "display_value" "text", "icon_link" "text")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION "public"."get_unresponded_events"() RETURNS TABLE("id" "uuid", "email_address" "text", "event_name" "text", "event_date_time" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
RETURN QUERY
...
AND e.date_time > NOW(); -- Only get events in the future
...
BEGIN
  ...
  SELECT
    jsonb_build_object(
      ...
      'event_date_time', v.event_date_time,
      'full_name', v.full_user_name,
  ...
  FROM
    public.view_event_attendance_details v
  WHERE
    v.event_id = event_id_param AND (v.response_id IS NULL OR v.response_id = 0);
END;
$$;
...
CREATE OR REPLACE FUNCTION "public"."get_unresponded_events"("_event_id" "uuid") RETURNS TABLE("id" "uuid", "email_address" "text", "event_name" "text", "event_date_time" timestamp with time zone)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  RETURN QUERY
  ...
  JOIN
    events e ON a.event_id = e.id
  WHERE
    a.event_id = _event_id -- Filter by the specific event_id passed to the function
    AND a.status = 'unresponded' -- Only include attendees who haven't responded
    AND e.date_time > NOW(); -- Ensure the event is in the future
...
CREATE OR REPLACE FUNCTION "public"."get_user_teams_debug"("user_id_param" "uuid") RETURNS TABLE("team_id" bigint, "team_name" "text")
    LANGUAGE "sql"
    AS $$
SELECT DISTINCT
    t.team_id,
...
JOIN
    public.member_team_link AS mtl ON uml.member_id = mtl.member_id
JOIN
    public.teams AS t ON mtl.team_id = t.team_id
WHERE
    uml.user_id = user_id_param;
$$;
...
BEGIN
    ...
    -- Colours for inline body text words — red for declined, green for accepted
    ...
    -- Badge label — always green regardless of response
    v_badge_label := CASE
    ...
    -- -------------------------------------------------------------------------
    -- STEP 6: Check admin count before attempting insert
    -- -------------------------------------------------------------------------
    ...
    RAISE NOTICE 'Admin users found for team: %', v_admin_count;

    IF v_admin_count = 0 THEN
        RAISE NOTICE 'WARN: No admin users (role_id=7) found for this team — no notifications will be created';
    END IF;

    -- -------------------------------------------------------------------------
    -- STEP 7: Insert one notification record per team admin
    -- -------------------------------------------------------------------------
    ...
    SELECT
        ...
        -- Status badge — label always green (#87C232)
...
CREATE TABLE IF NOT EXISTS "public"."match_score_types" (
    "id" bigint NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "score_type" "text",
    "team_id" bigint
...
ALTER TABLE "public"."match_score_types" OWNER TO "postgres";


ALTER TABLE "public"."match_score_types" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
...
CREATE TABLE IF NOT EXISTS "public"."match_scores_details" (
    "id" bigint NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "match_scores_id" bigint,
    "count" smallint,
    "score_type" bigint
);


ALTER TABLE "public"."match_scores_details" OWNER TO "postgres";


ALTER TABLE "public"."match_scores_details" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
...
CREATE OR REPLACE VIEW "public"."view_game_age_expansion" AS
 SELECT "g"."game_id",
    "g"."game_name",
    "g"."game_image",
    "g"."game_code",
    ...
    "g"."game_how_to_play",
    "g"."game_variations",
    "g"."game_teaching_points",
    "g"."game_pdf",
    "g"."game_video",
    "g"."game_link",
    "g"."game_details_image",
    "age"."age" AS "game_age_expanded"
   FROM "public"."games" "g",
    LATERAL "unnest"("string_to_array"("g"."game_age", ','::"text")) "age"("age");


ALTER VIEW "public"."view_game_age_expansion" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."match_score_types"
    ADD CONSTRAINT "match_score_types_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."match_scores_details"
...
CREATE INDEX "idx_games_game_age" ON "public"."games" USING "btree" ("game_age");
...
CREATE INDEX "idx_match_scores_details_score_id" ON "public"."match_scores_details" USING "btree" ("match_scores_id");



CREATE INDEX "idx_match_scores_details_score_type" ON "public"."match_scores_details" USING "btree" ("score_type");



CREATE INDEX "idx_match_scores_event_id" ON "public"."match_scores" USING "btree" ("event_id");



CREATE INDEX "idx_match_scores_squad_id" ON "public"."match_scores" USING "btree" ("squad_id");



CREATE INDEX "idx_match_scores_user_id" ON "public"."match_scores" USING "btree" ("user_id");
...
ALTER TABLE ONLY "public"."match_score_types"
    ADD CONSTRAINT "match_score_types_team_id_fkey" FOREIGN KEY ("team_id") REFERENCES "public"."teams"("team_id");



ALTER TABLE ONLY "public"."match_scores_details"
    ADD CONSTRAINT "match_scores_details_match_scores_id_fkey" FOREIGN KEY ("match_scores_id") REFERENCES "public"."match_scores"("id");



ALTER TABLE ONLY "public"."match_scores_details"
    ADD CONSTRAINT "match_scores_details_score_type_fkey" FOREIGN KEY ("score_type") REFERENCES "public"."match_score_types"("id");
...
GRANT ALL ON FUNCTION "public"."get_members_attendance_for_event"("p_event_id" bigint, "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_members_attendance_for_event"("p_event_id" bigint, "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_members_attendance_for_event"("p_event_id" bigint, "p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_unresponded_events"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_unresponded_events"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_unresponded_events"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_unresponded_events"("_event_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_unresponded_events"("_event_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_unresponded_events"("_event_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_user_teams_debug"("user_id_param" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_teams_debug"("user_id_param" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_teams_debug"("user_id_param" "uuid") TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_score_types" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_score_types" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_score_types" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores_details" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores_details" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."match_scores_details" TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."view_game_age_expansion" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."view_game_age_expansion" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."view_game_age_expansion" TO "service_role";
...
create sequence "public"."event_attendance_attendance_id_seq";

create sequence "public"."event_codes_code_id_seq";

create sequence "public"."event_responses_response_id_seq";

create sequence "public"."event_types_event_type_id_seq";

create sequence "public"."events_event_id_seq";

create sequence "public"."game_ages_id_seq";

create sequence "public"."games_game_id_seq";

create sequence "public"."member_team_link_member_team_id_seq";

create sequence "public"."user_member_link_user_member_id_seq";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_event_car_pools(p_event_id bigint, p_user_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 SECURITY DEFINER
AS $function$
...
-- 6. Final Assemble: Construct the root JSON object
SELECT
    jsonb_build_object(
        'current_user_has_pool', COALESCE(us.has_open_pool, false),
        'current_user_role_level', COALESCE(us.user_role_level, 0),
...
CREATE OR REPLACE FUNCTION public.get_events_list(p_date_from text DEFAULT NULL::text, p_date_to text DEFAULT NULL::text, p_team_id bigint DEFAULT NULL::bigint, p_code_id bigint DEFAULT NULL::bigint, p_type_id bigint DEFAULT NULL::bigint, p_opposition text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_user_clubs()
 RETURNS TABLE(user_id uuid, email_address text, user_first_name text, user_last_name text, user_full_name text, club_id bigint, club_name text, county text, banner text, crest text, sort_order integer)
 LANGUAGE sql
 SECURITY DEFINER
AS $function$
-- First part: Select all distinct clubs associated with the user via team links
SELECT DISTINCT
    u.user_id,
    u.email_address,
    u.first_name AS user_first_name,
...
WHERE
    u.user_id = auth.uid() -- SECURE: Filters by the currently authenticated user ID from the session JWT
-- Second part: Manually add the "All Clubs" option, linked only to the user
UNION ALL
SELECT
    u.user_id,
    u.email_address,
...
WHERE
    u.user_id = auth.uid() -- SECURE: Filters by the currently authenticated user ID from the session JWT
-- Final ordering logic: 1 (user_id), 11 (sort_order), 7 (club_name)
ORDER BY
    1,
    11,
    7;
...
-- Main Query: Wrap the results in json_agg and COALESCE
SELECT
    -- FIX: Cast json_agg result to jsonb to match the '[]'::jsonb type
    COALESCE(
        json_agg(final_result)::jsonb 
...
CREATE TRIGGER on_auth_user_confirmed AFTER UPDATE OF email_confirmed_at ON auth.users FOR EACH ROW WHEN (((old.email_confirmed_at IS NULL) AND (new.email_confirmed_at IS NOT NULL))) EXECUTE FUNCTION public.migrate_legacy_user_on_signup();
