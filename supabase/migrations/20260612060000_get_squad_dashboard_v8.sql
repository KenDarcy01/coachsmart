-- Squad dashboard v8: cap match_sels at NOW() — exclude future scheduled events
--
-- v7 bug: filtered_events includes any event within the slider date range,
--   including future events that have not yet been played.  Event 1007
--   (2026-06-13, tomorrow) already has a squad save, which inflated
--   match_count by 1 for every player selected for that event.
--   The drawer's deduplication keeps the most recent save per event and
--   only counts those already played (it excludes dt >= new Date() clientside),
--   so drawer and table were off by exactly 1 for all affected players.
--
-- v8 fix: add AND e.event_date_time < NOW() to filtered_events.
--   Only past events qualify for match-appearance counting.
--   Attendance stats (acceptance_rate, avg_accepted) are derived from
--   latest_att / member_att which use the slider range — unaffected.
...
BEGIN
    ...
    -- Only past events qualify for match-appearance counting.
    -- AND e.event_date_time < NOW() ensures a squad save for a future scheduled
    -- event does not inflate match_count before the match is played.
    filtered_events AS (
        SELECT e.event_id
        FROM public.events e
        JOIN club_teams ct               ON e.team_id        = ct.team_id
