-- ROLLBACK: Restore all functions to their state before migrations 000004-000012.
--
-- Apply this migration if the soft-delete / mtl.status changes cause problems:
--   supabase db push
--
-- DATA: This migration also resets any soft-deleted member_team_link rows
-- back to active. Only rows with status = 'removed' are affected (this value
-- was introduced by migration 000004 — no existing rows had this value before).
--
-- IMPORTANT: members.status rows set to 'deleted' are NOT automatically reset
-- here because some may pre-date this change. If you need to restore a
-- specific member, run manually:
--   UPDATE public.members SET status = 'active' WHERE member_id = <id>;

-- ── Data rollback ─────────────────────────────────────────────────────────────
UPDATE public.member_team_link SET status = 'active' WHERE status = 'removed';
...
ALTER FUNCTION public.remove_member_from_team(bigint, bigint) SECURITY DEFINER SET search_path = 'public';
...
ALTER FUNCTION public.get_user_team_summary(p_user_id uuid) SECURITY DEFINER SET search_path = 'public';
...
ALTER FUNCTION public.get_unresponded_events(event_id_param bigint) SECURITY DEFINER SET search_path = 'public';
...
ALTER FUNCTION public.get_unresponded_events_v2(event_id_param bigint, p_role_grade smallint, p_role_level smallint) SECURITY DEFINER SET search_path = 'public';
