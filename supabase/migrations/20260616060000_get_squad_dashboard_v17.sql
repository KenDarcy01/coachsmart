-- Squad dashboard v17
--
-- CHANGE — fix double-counting of events when a member appears in multiple
--   squads within the same save (e.g. Football Squad + Hurling Squad both
--   selected for a null-code training event).
--
--   v16 bug: match_sels groups by (member_id, team_id, squad_id).  If a
--   player is in Squad A AND Squad B for the same event in the same save,
--   match_count=1 per squad row.  The client summed these → total=2 for
--   one event, while the drawer's seenEvIds dedup counted it once → off
--   by 1 whenever "All" codes are selected.
--
--   Fix: add member_distinct_events CTE — COUNT(DISTINCT event_id) per
--   (member_id, team_id) across all latest saves.  This gives the true
--   number of distinct events a player was selected for, regardless of
--   how many squads they appeared in per event.
--
--   squad_member_rows now joins member_distinct_events and includes
--   total_event_count in each member's jsonb.  The client uses this field
--   for the per-player "total" instead of summing per-squad counts.
--
--   match_selections per squad is preserved unchanged (used for the
--   per-squad breakdown bars in the UI).
...
BEGIN
    ...
    -- Distinct events per (member, team) across ALL squads in latest saves.
    -- This is the true "how many events was this player selected for" count —
    -- immune to double-counting when a player appears in multiple squads per event.
    member_distinct_events AS (
        ...
        JOIN latest_saves ls
            ON  msd.event_id       = ls.event_id
            AND msd.match_squad_id = ls.match_squad_id
        GROUP BY msd.member_id, msd.team_id
    ),
