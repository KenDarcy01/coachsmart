-- Event-centric attendance dashboard RPC.
-- Returns all events for a team in a date range, each with the full
-- per-member response list embedded so the webview has everything it
-- needs for both the event list and the drill-through member view.

CREATE OR REPLACE FUNCTION public.get_team_event_dashboard(
    p_team_id    integer,
...
GRANT EXECUTE ON FUNCTION public.get_team_event_dashboard(integer, timestamptz, timestamptz)
