-- user_highest_role was derived from event_team_members which now carries
-- squad/grade/role-level filters. Decouple it so it always returns the
-- user's true highest role on the team, regardless of event filtering.
...
BEGIN
    ...
    -- Decoupled from event_team_members so filters on that CTE don't affect
    -- the user's true highest role on the team.
    user_highest_role AS (
        SELECT MAX(r.role_level) AS highest_role_level_for_user
        FROM public.members AS m
        INNER JOIN public.user_member_link AS uml ON m.member_id = uml.member_id
        INNER JOIN public.member_team_link AS mtl ON m.member_id = mtl.member_id
        INNER JOIN public.member_team_role_link AS mtrl ON mtl.member_team_id = mtrl.member_team_id
