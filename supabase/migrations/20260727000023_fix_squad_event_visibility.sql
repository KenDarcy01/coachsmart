-- Fix two bugs introduced by the squad-scoped event feature:
--
-- Bug 1: Admins cannot see squad-scoped events on the home page / events page.
--   Root cause: the lateral join that resolves the requesting user's member
--   (rm / relevant_member) filters by member_squad_link membership.  Admins
--   are never in member_squad_link, so the join returns NULL and the event is
--   hidden from the admin who created it.
--   Fix: add `OR r.role_grade = 100` — admin roles bypass the squad filter.
--   Affected RPCs: get_user_home_events, get_user_events, get_single_user_event.
--
-- Bug 2: get_events_list attendance counts include the full team for
--   squad-scoped events instead of only squad members.
--   Root cause: the all_team_members CTE has no squad_id filter.
--   Fix: add the same squad EXISTS check used in the other event RPCs.
--
-- BACKWARD COMPATIBLE: squad_id IS NULL on all current events, so existing
-- behaviour is unchanged until an admin creates a squad-scoped event.
...
all_team_members AS (
    ...
    WHERE (
      ...
      AND (                                        -- squad filter for counts
        fe.squad_id IS NULL
        OR EXISTS (
            SELECT 1 FROM public.member_squad_link msl
            WHERE msl.member_id = mtl.member_id
              AND msl.squad_id  = fe.squad_id
              AND msl.team_id   = fe.team_id
        )
    )
