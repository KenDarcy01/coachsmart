-- Add a guard to create_new_member_by_code for the case where the calling
-- user already has an active user_member_link to a confirmed member with the
-- same name on the same team.  Previously the NOT EXISTS in Check 2 excluded
-- such members from the duplicate search, causing a duplicate member record
-- to be created instead of returning a clear already-linked status.
--
-- New return status: 'already_member'
-- Webview should show a message: "You're already linked to this person."
...
BEGIN
    ...
    -- Check 0: this user already has an active link to a confirmed member with
    -- this name on this team — return early rather than creating a duplicate.
    IF EXISTS (
        SELECT 1
    ...
    -- name on the team.  Excludes members this user already owns (covered above).
