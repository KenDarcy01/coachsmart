-- Squad dashboard v16
--
-- CHANGE — fix match_sels to use match_squads (not match_squad_details) to find
--   the latest save per event, exactly mirroring the client-side drawer logic:
--
--   1. latest_saves: DISTINCT ON (event_id) from match_squads joined to
--      filtered_events, ordered by match_squad_id DESC → single authoritative
--      latest save per event.
--
--   2. match_sels: joins match_squad_details to latest_saves on BOTH event_id
--      AND match_squad_id → only rows from the latest save survive.
--
--   v14 was wrong because it used match_squad_details to find the latest
--   match_squad_id per event (DISTINCT ON event_id from msd) — this gave
--   different results from the authoritative match_squads table.
--   v15 reverted to v12 (no dedup at all) which overcounted.
--   v16 uses match_squads as the canonical source, matching the drawer exactly.
--
-- All other v15 content is preserved (team_female, squad_assignments DISTINCT,
-- member_att groups by member_id/team_id only).
