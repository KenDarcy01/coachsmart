-- Add club_pathway bool to get_user_home_events.
-- true  = users.default_club has at least one row in club_pathway_link
-- false = no pathway linked (or default_club is NULL)
...
user_details AS (
    SELECT
        ...
        EXISTS (
            SELECT 1 FROM public.club_pathway_link cpl
            WHERE cpl.club_id = u.default_club
        ) AS club_pathway
...
final_events_data AS (
    SELECT
        ...
        ue.status AS event_status, ue.squad_id, ue.squad_name, ue.squad_image,
