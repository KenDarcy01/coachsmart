-- Squad dashboard RPC. Rewritten as LANGUAGE sql with single-quote body
-- to avoid Supabase CLI dollar-quote parsing issues on large functions.
...
'WITH
club_info AS (
    SELECT club_name FROM public.clubs WHERE club_id = $1
),
club_teams AS (
    SELECT t.team_id, t.team_name
    FROM public.teams t WHERE t.club_id = $1
),
...
latest_att AS (
    ...
    WHERE e.event_date_time >= $2
      AND e.event_date_time <  $3
...
SELECT
    CASE WHEN NOT EXISTS (SELECT 1 FROM club_info)
    THEN jsonb_build_object(''error'', ''Club not found'')
    ELSE jsonb_build_object(
        ''club_name'',    (SELECT club_name FROM club_info),
        ''club_id'',      $1,
        ''start_date'',   $2,
        ''end_date'',     $3,
        ''generated_at'', now(),
