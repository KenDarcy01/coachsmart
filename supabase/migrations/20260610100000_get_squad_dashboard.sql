-- Squad dashboard RPC. Follows same pattern as get_club_comparison_dashboard:
-- LANGUAGE plpgsql, $$ delimiter, named params, nested data pre-computed in CTEs.
...
BEGIN
    ...
    squad_member_rows AS (
        -- One row per (team, squad, code): pre-computed member JSON array
        SELECT
            sa.team_id,
            sa.squad_id,
            sa.code_id,
    ...
    squad_rows AS (
        -- One row per team: pre-computed squad JSON array
        SELECT
            ss.team_id,
            COALESCE(
                jsonb_agg(
