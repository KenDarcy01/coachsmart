-- Deduplicate matchSelections: one row per event (member can appear in multiple
-- match_squads/match_squad_details for the same event). Use DISTINCT ON (event_id)
-- ordering by msd.id DESC so the most recently added detail row wins.
-- Also add a defensive outer DISTINCT ON (event_id) to the events query.
...
BEGIN
  RETURN jsonb_build_object(
    ...
    -- Outer DISTINCT ON (event_id) as a defensive layer on top of the inner one
    'events', (
      SELECT COALESCE(
        jsonb_agg(
          jsonb_build_object(
    ...
    -- DISTINCT ON (event_id) — one row per event, most recent squad detail wins
    'matchSelections', (
      SELECT COALESCE(
        jsonb_agg(
          jsonb_build_object(
      ...
      FROM (
        ...
        ORDER BY e.event_id, msd.id DESC
      ) ms
    )
