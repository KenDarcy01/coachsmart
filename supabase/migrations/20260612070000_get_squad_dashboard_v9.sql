-- Squad dashboard v9: revert v8 NOW() cap — include future events with squad saves
--
-- v8 introduced AND e.event_date_time < NOW() in filtered_events, which excluded
-- future scheduled events (e.g. event 1007, June 13) from match_count even when
-- a squad has already been saved for them.  The intent was wrong: if a coach has
-- saved a squad for tomorrow's match it should count as a selection.
--
-- v9 reverts to v7 logic (COUNT DISTINCT event_id, no latest_squad_saves CTE)
-- with no NOW() cap.  The slider end date is the only upper bound.
