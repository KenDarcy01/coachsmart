-- Squad dashboard v3:
-- 1. Add p_att_min / p_att_max  — attendance threshold (min/max accepted count per event)
-- 2. Rebase squad_member_rows on match_sels (actual selections) not member_squad_link
--    so players selected for matches but absent from the squad roster are included
-- 3. Two targeted performance indexes for the two hot-path scans

CREATE INDEX IF NOT EXISTS idx_event_att_dedup
    ON public.event_attendance (event_id, member_id, attendance_id DESC);

CREATE INDEX IF NOT EXISTS idx_msd_team_event_squad_member
    ON public.match_squad_details (team_id, event_id, squad_id, member_id);
...
BEGIN
    ...
    -- Total accepted count per event — used for attendance threshold filtering.
    -- Counts all responses, not code/type filtered, so the threshold reflects
    -- true event attendance regardless of which codes are being analysed.
    event_att_counts AS (
        SELECT
            e.event_id,
    ...
    match_sels AS (
        ...
        JOIN event_att_counts eac        ON msd.event_id     = eac.event_id
        LEFT JOIN public.event_codes ec  ON e.event_code_id  = ec.code_id
        LEFT JOIN public.event_types et  ON e.event_type_id  = et.event_type_id
        WHERE e.event_date_time >= p_start_date
          AND e.event_date_time <  p_end_date
    ...
    -- Member rows sourced from match_sels so players selected for matches
    -- but not registered in member_squad_link are included.
    -- Keyed by (team_id, squad_id) — no code_id — one list per squad.
