-- RPC for the team attendance dashboard webview.
-- Returns all player attendance data for a team over a date range in one
-- call so the webview has everything it needs for all drill-through levels
-- without further round trips.
--
-- Players are defined as members with role_grade = 10 on the team.
-- Events are past events only (event_date_time < p_end_date).
-- Attendance uses the latest record per member/event (handles re-responses).
...
BEGIN
    ...
    team_players AS (
        -- One row per player (DISTINCT ON handles members with multiple roles)
    ...
    latest_attendance AS (
        -- Keep only the most recent response per member/event
    ...
    player_stats AS (
        ...
        GROUP BY tp.member_id, tp.full_name
    )
