-- Squad dashboard v4:
-- 1. Fix count inflation: deduplicate by (member_id, event_id) keeping the
--    most recent match_squad_id before counting per squad.
--    Mirrors the byEvent Map deduplication the client drawer already applies.
-- 2. Exclude blank-named members from squad_member_rows.
...
BEGIN
    ...
    -- Deduplicate: one row per (member_id, event_id), keeping the most recent
    -- match_squad_id. This matches what the client drawer byEvent Map does.
    match_sels_base AS (
        SELECT DISTINCT ON (msd.member_id, msd.event_id)
            msd.member_id,
            msd.team_id,
            msd.squad_id,
            msd.event_id
        ...
        WHERE e.event_date_time >= p_start_date
          ...
          AND (p_att_min <= 0     OR eac.accepted_count >= p_att_min)
          AND (p_att_max  IS NULL OR eac.accepted_count <= p_att_max)
        ORDER BY msd.member_id, msd.event_id, msd.match_squad_id DESC
    ),
    match_sels AS (
        SELECT member_id, team_id, squad_id, COUNT(*) AS match_count
        FROM match_sels_base
        GROUP BY member_id, team_id, squad_id
    ),
    squad_stats AS (
        SELECT
            sa.team_id,
    ...
    -- Sourced from match_sels (actual selections). Blank names excluded via FILTER.
