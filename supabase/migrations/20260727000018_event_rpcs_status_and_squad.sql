-- Rewrite event-facing RPCs to incorporate:
--   1. Status filtering  — 'deleted' events hidden everywhere; 'cancelled'
--      events remain visible (shown with cancelled treatment) until the
--      event date passes naturally.
--   2. Squad-scoped visibility — if events.squad_id IS NOT NULL, only
--      members who belong to that squad (via member_squad_link) can see
--      the event. squad_id IS NULL = whole-team visibility (current default).
--   3. Squads returned in create/edit RPCs so FlutterFlow can show a
--      squad picker when creating or editing an event.
--   4. event_status and squad_id returned in response payloads so the app
--      can render cancelled events differently and round-trip squad_id.
--
-- No behaviour change for existing data — all current events have
-- status = 'active' and squad_id = NULL, so filters pass everything through.
...
upcoming_events AS (
    ...
    WHERE e.event_date_time >= CURRENT_DATE
      AND e.status != 'deleted'                   -- hide deleted events
),
...
final_events_data AS (
    SELECT
        ...
        ue.status AS event_status, ue.squad_id,                -- new fields
    ...
    LEFT JOIN (
        ...
        WHERE r.role_level >= ue2.event_role_level
          AND (                                    -- squad filter for the user's member
...
all_team_members AS (
    ...
    WHERE (
      ...
      AND (                                        -- squad filter for attendance counts
        ue.squad_id IS NULL
        OR EXISTS (
            SELECT 1 FROM public.member_squad_link msl
            WHERE msl.member_id = mtl.member_id
...
final_event_data AS (
    SELECT
        ...
        te.status AS event_status, te.squad_id,              -- new fields
        rm.member_id, lar_user.attendance_id,
        ert.display_value AS attendance_status, ert.icon_link AS attendance_icon,
...
-- Deleted events are hidden; cancelled events remain visible so admins
-- can see them. status is returned so the admin UI can render it.
...
filtered_events AS (
    ...
    WHERE e.event_date_time_2 >= COALESCE(sf.filter_date_from::timestamp with time zone, '1900-01-01'::timestamp with time zone)
      ...
      AND e.status != 'deleted'                   -- hide deleted; cancelled still shown
),
all_team_members AS (
...
FROM (
    SELECT
        ...
        fe.status AS event_status,                            -- new field
...
-- Adds a 'squads' array to each team so FlutterFlow can show a squad picker.
...
-- Adds squads per team + returns squad_id and status for the current event.
