-- Add delivery_method = 'push' to notification INSERTs in all four approval
-- RPCs. Without this the cron job fetches the rows but skips them (neither
-- push nor email branch matches NULL), so nothing is ever sent.
--
-- Functions updated:
--   confirm_member_join(bigint, bigint[])   — "Request Approved"
--   deny_member_join(bigint)                — "Request Not Approved"
--   confirm_user_member_access(bigint)      — "Access Approved"
--   deny_user_member_access(bigint)         — "Access Request Not Approved"
