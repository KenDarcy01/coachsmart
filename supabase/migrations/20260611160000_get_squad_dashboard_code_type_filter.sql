-- Add p_codes and p_types filter parameters to get_squad_dashboard so that
-- the Code and Type filter chips on the Squads tab apply to match_selections
-- at the database level, exactly as they do on every other tab.
--
-- p_codes text[]  DEFAULT NULL — filter events by event_code (e.g. ['SEN','MIN'])
-- p_types text[]  DEFAULT NULL — filter events by event_type (e.g. ['Match','League'])
-- NULL means "no filter — include all".
...
BEGIN
    ...
    squad_member_rows AS (
        ...
        LEFT JOIN team_member_roles tmr
            ON  sa.member_id = tmr.member_id
            AND sa.team_id   = tmr.team_id
        LEFT JOIN member_att ma
            ON  sa.member_id = ma.member_id
            AND sa.team_id   = ma.team_id
            AND sa.code_id   = ma.code_id
        LEFT JOIN match_sels ms
            ...
            AND sa.team_id   = ms.team_id
            AND sa.squad_id  = ms.squad_id
        GROUP BY sa.team_id, sa.squad_id, sa.code_id
    ),
    squad_rows AS (
        SELECT
            ss.team_id,
...
GRANT EXECUTE ON FUNCTION public.get_squad_dashboard(integer, timestamptz, timestamptz, text[], text[])
