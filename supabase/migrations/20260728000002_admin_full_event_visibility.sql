-- Complete admin visibility bypass in get_user_home_events.
--
-- Previously the rm subquery required r.role_level >= event_role_level
-- for all users, then bypassed the squad check for grade=100.
-- An admin whose role_level was below the event audience level could
-- still be excluded. Now grade=100 bypasses BOTH checks: admins see
-- all team events regardless of audience level or squad assignment.
-- Counts (eligible_attendees) are unaffected — grade=100 was already
-- excluded from there.
