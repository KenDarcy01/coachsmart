-- Rework create_team_with_admin: create one "No Team" squad per club code
-- (from club_code_link) and link the new member to each squad with the
-- corresponding code_id. Falls back to a single squad with null code_id if
-- the club has no codes configured.
...
DECLARE
  ...
  v_squad_id       bigint;   -- first squad created; used for member_team_link
  v_sq             bigint;
  v_code_rec       record;
  v_squad_ids      bigint[];
  v_code_ids       bigint[];
...
BEGIN
  ...
  -- 2. One "No Team" squad per club code
  FOR v_code_rec IN
  ...
  LOOP
    ...
    v_squad_ids := array_append(v_squad_ids, v_sq);
    v_code_ids  := array_append(v_code_ids, v_code_rec.code_id);
  END LOOP;

  -- Fallback: club has no codes configured
  IF v_squad_ids IS NULL THEN
    INSERT INTO squads (team_id, squad_name, grade, squad_colour, squad_list_seq)
    VALUES (v_team_id, 'No Team', '', '#ffffff', 100)
    RETURNING squad_id INTO v_sq;
    v_squad_ids := ARRAY[v_sq];
    v_code_ids  := ARRAY[NULL::bigint];
  END IF;

  v_squad_id := v_squad_ids[1];  -- first squad for member_team_link
  ...
  VALUES (v_member_id, v_team_id, v_squad_id, v_team_code, 'active')
  ...
  -- 8. Link member to each squad (one per club code)
  FOR i IN 1..array_length(v_squad_ids, 1) LOOP
    INSERT INTO member_squad_link (member_id, squad_id, team_id, code_id)
    VALUES (v_member_id, v_squad_ids[i], v_team_id, v_code_ids[i]);
  END LOOP;
