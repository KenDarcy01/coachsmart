-- Squad dashboard v5: correct match_sels logic + squad_list_seq in JSON
--
-- v3 bug: COUNT(DISTINCT event_id) over all match_squad_details counted rows
--   from old revisions of the squad save (same event, lower match_squad_id).
--
-- v4 bug: DISTINCT ON (member_id, event_id) deduped at the details level,
--   so a player in BOTH "Starting 15" AND "Subs" in the LATEST save was
--   collapsed to one squad — producing counts that were too low.
--
-- v5 fix: deduplicate at the match_squads level (one match_squad per event,
--   keeping the highest match_squad_id), then take ALL match_squad_details
--   from those latest saves.  A player can appear in multiple squads within
--   the same save (legitimate); old revision rows are automatically excluded.
--
-- Also: include squad_list_seq in squad JSON so the JS column order is
--   driven by the squads table sequence rather than alphabetical fallback.

CREATE INDEX IF NOT EXISTS idx_match_squads_event_squad_desc
    ON public.match_squads (event_id, match_squad_id DESC);
...
BEGIN
    ...
    -- Step 1: events that pass all date/code/type/attendance filters.
    filtered_events AS (
        SELECT e.event_id
        FROM public.events e
        JOIN club_teams ct               ON e.team_id        = ct.team_id
    ...
    -- Step 2: for each filtered event keep only the most recent match_squad
    -- (highest match_squad_id = latest save; older revisions are discarded).
    latest_match_squads AS (
    ...
    -- Step 3: all squad selections from those latest saves.
    -- A player can legitimately appear in multiple squads within the same save
    -- (e.g. "Starting 15" + "Subs panel"), so no deduplication here.
    match_sels AS (
        SELECT
            msd.member_id,
            msd.team_id,
        ...
        JOIN latest_match_squads lms ON msd.match_squad_id = lms.match_squad_id
