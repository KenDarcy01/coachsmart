-- Squad dashboard v15
--
-- CHANGE 1 — revert match_sels to v12's COUNT(DISTINCT event_id) approach (no save-version dedup):
--   The latest_squad_saves CTE introduced in v14 performed a DISTINCT ON (event_id) to identify
--   the single highest match_squad_id per event as the "canonical" save. However this incorrectly
--   excluded squads from events where a newer partial save existed for a *different* squad colour:
--   if a coach re-saved one colour group, the old match_squad_id for another colour became
--   invisible, dropping those players entirely. v15 removes latest_squad_saves and joins
--   match_squad_details directly on filtered_events, counting COUNT(DISTINCT event_id) per
--   (member, team, squad) — the same approach used in v12.
--
-- CHANGE 2 — add team_female to club_teams CTE and team JSON output:
--   team_female is now selected in the club_teams CTE and included in the jsonb_build_object
--   for each team row, enabling client-side Hurling→Camogie display logic.
--
-- All other v12/v13/v14 fixes are preserved:
