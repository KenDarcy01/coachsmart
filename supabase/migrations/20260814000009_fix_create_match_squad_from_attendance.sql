-- Fix create_match_squad_from_attendance:
--
-- 1. Idempotency: delete any existing squad(s) for the event before creating a
--    fresh one. Without this, each call appended a new match_squads row and
--    duplicate detail rows accumulated.
--
-- 2. mtl.status = 'active': align with every other attendance RPC. Soft-deleted
--    members (status = 'removed') are excluded from the squad.
--
-- 3. Squad-scope filter: if the event has squad_id set, only include players who
--    belong to that squad (matching the squad + effective code). All other
--    attendance RPCs respect this filter; this one did not.
--
-- 4. Deterministic role_id: use a correlated subquery that picks the grade-10
--    role with the highest role_level (lowest role_id as tiebreaker). Previously
--    DISTINCT ON with only created_at in ORDER BY made the role_id arbitrary when
--    a member held two grade-10 roles. Members with no grade-10 role (managers
--    etc.) get role_id = NULL and are skipped.
--
-- 5. NULL guard: bail out cleanly if the event does not exist (v_team_id IS NULL).
...
BEGIN
    ...
    -- 2. Resolve effective squad code for this event
    v_lookup_code_id := public.get_updated_event_code(p_event_id);

    -- 3. Delete any existing squad(s) for this event (idempotent rebuild)
    DELETE FROM public.match_squad_details
     WHERE match_squad_id IN (
         SELECT match_squad_id FROM public.match_squads WHERE event_id = p_event_id
     );
    DELETE FROM public.match_squads WHERE event_id = p_event_id;

    -- 4. Create the fresh parent record
    INSERT INTO public.match_squads (event_id, user_id)
    VALUES (p_event_id, p_user_id)
    RETURNING match_squad_id INTO v_new_match_squad_id;

    -- 5. Loop over the most recent response per player who is still active on the team
    FOR v_member_record IN
        SELECT DISTINCT ON (ea.member_id)
            ea.member_id,
            ea.response_id,
            -- Deterministic grade-10 role: highest role_level, lowest role_id as tiebreaker
            (
        ...
        WHERE ea.event_id    = p_event_id
          ...
          AND (
              -- Squad-scope: if event is restricted to a squad, only include its members
    ...
    LOOP
        -- 6. Only include players who accepted (response_id = 3) and have a grade-10 role
