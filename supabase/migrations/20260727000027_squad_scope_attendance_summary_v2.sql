-- Update get_event_attendance_summary_by_role_and_squad_v2 with two internal fixes:
--
-- 1. Squad scoping: when events.squad_id IS NOT NULL, restrict both the member
--    population (actual_member_squad_roles_for_event) and the squad display list
--    (all_squad_roles_for_event) to that squad only. Events without a squad_id
--    continue to show all squads (existing behaviour, zero breaking change).
--
-- 2. Role grade filter: add JOIN to roles in actual_member_squad_roles_for_event
--    and apply AND r.role_grade = 10 so admin-grade roles (grade=100) are never
--    included in the attendance counts. Dual-role members (Admin + Coach) are now
--    counted once via their grade=10 role only.
