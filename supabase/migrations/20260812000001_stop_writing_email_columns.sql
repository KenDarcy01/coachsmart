-- Stop writing to email_title / email_body in notification-creating functions.
-- HTML is now generated at send time in cron_batch_notification_send using
-- app_title / app_body, so pre-populating the blob columns is unnecessary.
-- Columns are retained in the table for safe rollback.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. notify_admins_attendance_change
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- -------------------------------------------------------------------------
    -- STEP 6: Check admin count before attempting insert (excluding actor)
    -- -------------------------------------------------------------------------
    ...
    -- -------------------------------------------------------------------------
    -- STEP 7: Insert one notification record per team admin (excluding actor)
    -- -------------------------------------------------------------------------
    ...
    SELECT
        ...
        -- is_read: true for email (user sees it in inbox), false for push
        CASE
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. populate_event_notifications
-- ─────────────────────────────────────────────────────────────────────────────
