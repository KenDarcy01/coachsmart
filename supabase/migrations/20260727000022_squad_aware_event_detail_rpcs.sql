-- Fix attendance counts in event-detail RPCs to be squad-aware and apply the
-- member status filter.
--
-- BACKWARD COMPATIBLE: all existing events have squad_id = NULL so the filter
--   AND (squad_id IS NULL OR EXISTS (...))
-- is always TRUE for current data — zero behaviour change until an admin
-- creates an event with a squad selected.
--
-- Changes per RPC:
--   get_user_event_details
--     • Add squad_id + event_status to event_details CTE
--     • Squad filter + m.status != 'deleted' on categorized_eligible_members
--     • strictly_matched_members derived from categorized_eligible_members
--       (inherits squad filter automatically)
--     • m.status != 'deleted' on event_team_members
--     • event_status, squad_id, squad_name added to JSON output
--
--   get_event_attendance_summary_by_role
--     • Squad filter + m.status != 'deleted' on actual_member_roles_for_event
...
WITH event_details AS (
    SELECT
        e.event_id,
        e.team_id,
        e.squad_id,                         -- squad filter anchor
        e.status        AS event_status,    -- for cancelled display
        e.meet_time,
        e.event_link,
        e.location_name,
        e.location_pin,
...
categorized_eligible_members AS (
    -- Members eligible to attend: correct role, not deleted, and in the event's
    -- squad if squad_id is set (squad_id IS NULL = whole team, always passes).
...
strictly_matched_members AS (
    -- Exact role-level match; derived from categorized_eligible_members so the
    -- squad filter is inherited automatically.
...
BEGIN
    ...
    actual_member_roles_for_event AS (
        -- Members eligible for this event: on the team, not deleted, and in
        -- the event's squad when squad_id is set (NULL = whole team).
        SELECT DISTINCT
            e.event_id,
            mtl.member_id,
            mtrl.role_id
