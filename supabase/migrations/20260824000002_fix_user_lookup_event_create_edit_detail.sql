-- get_user_event_create_detail and get_user_event_edit_detail used
-- m.user_id = p_user_id to locate the caller's member records.
-- For users whose members.user_id is NULL (linked only via user_member_link),
-- this returns no rows → create_teams is null → edit/create form appears blank.
-- get_user_event_details already uses user_member_link correctly.
-- Fix: replace the direct m.user_id filter with a JOIN through user_member_link
-- in both the v_default_team_id query and the create_teams subquery.
