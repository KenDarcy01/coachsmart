-- Bug fix: populate_event_notifications was hardcoding is_read = false for ALL rows,
-- including email-only users (no FCM token). This caused email-delivered reminder
-- notifications to accumulate permanently in the unread badge count.
-- Fix: mirror the same FCM-token check used in notify_admins_attendance_change.
...
BEGIN
    ...
    SELECT
        ...
        -- is_read: email lands in inbox so mark read immediately; push stays unread until user taps
        CASE
