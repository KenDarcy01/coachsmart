-- Replace parameter names in error messages with human-readable equivalents.
...
BEGIN
    ...
    IF p_recurring_type NOT IN (0, 1, 2) THEN
        RETURN QUERY SELECT false, 0, '{}'::BIGINT[],
            'Recurring type must be None, Weekly or Fortnightly.'::TEXT;
        RETURN;
    END IF;

    IF p_event_date_time IS NULL THEN
        RETURN QUERY SELECT false, 0, '{}'::BIGINT[],
            'Event date and time is required.'::TEXT;
        RETURN;
    END IF;
    ...
    IF p_recurring_type = 0 THEN
        ...
        RETURN QUERY SELECT true, 1, created_ids,
    ...
    IF p_num_weeks < 1 OR p_num_weeks > 12 THEN
        RETURN QUERY SELECT false, 0, '{}'::BIGINT[],
            'Number of weeks must be between 1 and 12 for recurring events.'::TEXT;
        RETURN;
    END IF;
