-- Squad dashboard v13: restore latest-save deduplication from v6
--
-- v12 bug (inherited from v7): match_sels reads ALL rows in match_squad_details
--   for filtered events without any revision deduplication.  When a coach
--   re-saves a squad after a player declines, the old save row remains in
--   match_squad_details and the player still shows up in the table even though
--   they are not in the final committed squad.
--
-- v13 fix: restore v6's latest_squad_saves CTE.
--   DISTINCT ON (event_id, squad_id) ordered by match_squad_id DESC picks the
--   highest match_squad_id for each (event, squad) pair — the latest save.
--   match_sels then joins to match_squad_details filtered to those latest-save
--   rows only, so removed players are correctly excluded.
--
--   This correctly handles both cases:
--     a) Multiple squad types per event (Blue/Red/Green each tracked separately
--        via the squad_id dimension in DISTINCT ON).
--     b) Multiple revision saves of the same squad (only the latest is used).
--
--   All other v12 fixes are preserved:
...
BEGIN
    ...
    -- For each (event_id, squad_id) keep the highest match_squad_id = latest save.
    -- DISTINCT ON (event_id, squad_id) handles multiple squad colours per event
    -- and multiple revision saves of the same squad simultaneously.
    ...
    -- Count events per (member, team, squad) using only latest-save rows.
    -- Players removed in a re-save are excluded; players in multiple squad
    -- colours within the same save are counted once per squad colour.
    match_sels AS (
        SELECT
            msd.member_id,
            msd.team_id,
