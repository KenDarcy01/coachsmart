-- Squad dashboard v12: fix squad doubling + broken KPIs
--
-- Root causes fixed:
-- 1) squad_assignments included code_id from member_squad_link. When members
--    of the same squad had different code_ids (League/Challenge/NULL) the CTE
--    produced one row per (squad × code), so squad_stats emitted the same squad
--    multiple times — doubling/tripling KPIs and card counts.
--    Fix: DISTINCT ON (team_id, squad_id, member_id) in squad_assignments,
--    dropping code_id entirely from that CTE.
--
-- 2) member_att was grouped by (member_id, team_id, code_id) where code_id
--    was the EVENT's event_code_id. squad_stats joined it on sa.code_id = ma.code_id
--    where sa.code_id was the MEMBER's squad-link code_id. These two domains
--    rarely matched, so acceptance_rate / avg_accepted were nearly always 0.
--    Fix: member_att groups by (member_id, team_id) only; squad_stats joins
--    without any code_id restriction.
--
-- 3) squad_rows emitted 'code' / 'code_id' fields that are no longer meaningful
--    after removing code_id from squad grouping. Removed from output.
...
BEGIN
    ...
    -- One row per (team, squad, member) — drop code_id to prevent fan-out
