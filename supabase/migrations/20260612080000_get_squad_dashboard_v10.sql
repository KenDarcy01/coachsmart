-- Squad dashboard v10: re-apply NOW() cap after v9 accidentally reverted it
--
-- v8 correctly added AND e.event_date_time < NOW() to filtered_events so that
-- squad saves for future scheduled events do not inflate match_count.
-- v9 reverted that cap in error.  v10 restores the v8 behaviour.
--
-- The drawer's inRange filter has a matching dt >= new Date() guard so both
-- table and drawer consistently exclude future events.
...
BEGIN
    ...
    -- Only past events count toward match appearances.
    -- AND e.event_date_time < NOW() prevents a squad save for a future scheduled
    -- match from inflating match_count before it has been played.
    filtered_events AS (
        SELECT e.event_id
        FROM public.events e
        JOIN club_teams ct               ON e.team_id        = ct.team_id
