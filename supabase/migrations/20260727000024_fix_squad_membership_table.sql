...
--
-- Root cause: migrations 000018–000023 checked squad membership via
-- member_squad_link (the per-event-code squad dashboard table).  The actual
-- squad-per-member data lives on member_team_link.squad_id.  The correct
-- filter is simply:
--
--   AND (event.squad_id IS NULL OR mtl.squad_id = event.squad_id)
--
-- This migration:
--   1. Rewrites get_user_home_events, get_user_events, get_single_user_event,
--      get_events_list (previously fixed in 000023 but with wrong table).
--   2. Rewrites get_user_event_details, get_event_attendance_summary_by_role
--      (previously fixed in 000022 but with wrong table).
--   3. Updates get_event_attendance_by_role and get_event_attendance_by_role_v2
--      to filter member lists by events.squad_id when it is set (never
--      previously filtered).
--
-- BACKWARD COMPATIBLE: all current events have squad_id = NULL so the
-- condition (event.squad_id IS NULL OR ...) is always TRUE — zero behaviour
-- change until an admin creates a squad-scoped event.
...
eligible_attendees AS (
    ...
    WHERE (
      ...
      AND (
        ue.squad_id IS NULL
        OR mtl.squad_id = ue.squad_id          -- use member_team_link.squad_id
    )
),
    ...
    LEFT JOIN (
        ...
        WHERE r.role_level >= ue2.event_role_level
          AND (
            ...
            OR mtl.squad_id = ue2.squad_id      -- player in this squad
        )
...
relevant_member AS (
    ...
    WHERE r.role_level >= ue.event_role_level
      AND (
        ue.squad_id IS NULL
        OR r.role_grade = 100               -- admins always see squad-scoped events
        OR mtl.squad_id = ue.squad_id       -- player in this squad
    )
...
categorized_eligible_members AS (
    ...
    WHERE mtl.team_id = ed.team_id
      ...
      AND (
        ed.squad_id IS NULL
        OR mtl.squad_id = ed.squad_id          -- use member_team_link.squad_id
    )
),
...
BEGIN
    ...
    actual_member_roles_for_event AS (
        ...
        WHERE m.status != 'deleted'
          AND (
            e.squad_id IS NULL
            OR mtl.squad_id = e.squad_id       -- use member_team_link.squad_id
        )
    ),
    latest_event_attendance AS (
        SELECT DISTINCT ON (ea.event_id, ea.member_id)
...
-- Add filter: only return members in the event's squad when events.squad_id
-- is set.  Existing behaviour (squad_id IS NULL = show all squad members)
-- is preserved.
...
BEGIN
    ...
    member_base_data AS (
        ...
        JOIN public.member_team_role_link mtrl ON mtl.member_team_id = mtrl.member_team_id
        WHERE mtl.squad_id IS NOT NULL
          AND (e.squad_id IS NULL OR mtl.squad_id = e.squad_id)   -- squad filter
    ),
    ...
    member_status_data AS (
        SELECT
            mbd.event_id, mbd.role_id, mbd.first_name, mbd.last_name,
            mbd.squad_grade, mbd.squad_list_seq, lea.response_id,
            jsonb_build_object(
    ...
    roles_with_attendance AS (
        SELECT
            r.role_id, r.role_name, r.role_name_plural, r.role_grade, r.role_level, r.role_list_seq,
        ...
        jsonb_agg(
            CASE
                WHEN p_response_id = 3 THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'member_count', jsonb_array_length(ra.accepted_members),
                        'members', ra.accepted_members)
                WHEN p_response_id = 4 THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'member_count', jsonb_array_length(ra.declined_members),
                        'members', ra.declined_members)
                WHEN p_response_id IS NOT NULL AND p_response_id NOT IN (3, 4) THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'member_count', jsonb_array_length(ra.no_response_members),
...
-- Add filter: only return members in the event's squad when events.squad_id
-- is set.  Uses member_squad_link (code-based) for squad detail; filters
-- msl.squad_id = e.squad_id when the event has a squad assigned.
...
BEGIN
    ...
    member_base_data AS (
        ...
        WHERE e.event_id = p_event_id
          ...
          AND (e.squad_id IS NULL OR msl.squad_id = e.squad_id)   -- squad filter
    ),
    all_roles_for_event AS (
        SELECT DISTINCT
            e.event_id,
    ...
    member_status_data AS (
        SELECT
            mbd.event_id, mbd.role_id, mbd.member_id, mbd.first_name, mbd.last_name,
        ...
        jsonb_agg(
            CASE
                WHEN p_response_id = 3 THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'squad_list_seq', ra.min_squad_seq,
                        'member_count', jsonb_array_length(ra.accepted_members),
                        'member_role_count', ra.code_accepted_count, 'members', ra.accepted_members)
                WHEN p_response_id = 4 THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'squad_list_seq', ra.min_squad_seq,
                        'member_count', jsonb_array_length(ra.declined_members),
                        'member_role_count', ra.code_declined_count, 'members', ra.declined_members)
                WHEN p_response_id IS NOT NULL AND p_response_id NOT IN (3, 4) THEN
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'squad_list_seq', ra.min_squad_seq,
                        'member_count', jsonb_array_length(ra.no_response_members),
                        'member_role_count', ra.code_no_response_count, 'members', ra.no_response_members)
                ELSE
                    jsonb_build_object('role_id', ra.role_id, 'role_name', ra.role_name,
                        'role_name_plural', ra.role_name_plural, 'role_grade', ra.role_grade,
                        'role_level', ra.role_level, 'squad_list_seq', ra.min_squad_seq,
