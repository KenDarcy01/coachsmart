-- Squad dashboard v2 — server-side aggregation via SQL RPC
--
-- Replaces the chunked REST approach with two functions:
--
--   get_squad_dashboard_v2  — returns the full teams → squads → members
--                             aggregate tree (same shape as v18) but accepts
--                             an optional p_team_ids filter so callers can
--                             restrict to specific teams without a second query.
--
--   get_player_squad_events_v2 — returns a JSONB array of per-event squad
--                             appearances for a single player. Called on-demand
--                             when the drawer opens. Returns O(events) rows, not
--                             O(players × events), so dataset size is irrelevant.
--
-- Both functions are SECURITY DEFINER with search_path pinned.
-- Neither is affected by the PostgREST max_rows cap (they return a single
-- JSONB value, not a raw table scan).

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. get_squad_dashboard_v2
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.get_squad_dashboard_v2(
    p_club_id    integer,
...
GRANT EXECUTE ON FUNCTION public.get_squad_dashboard_v2(integer, timestamptz, timestamptz, integer[], text[], text[], integer, integer)
    TO anon, authenticated, service_role;


-- ─────────────────────────────────────────────────────────────────────────────
-- 2. get_player_squad_events_v2
-- ─────────────────────────────────────────────────────────────────────────────
-- Returns a JSONB array (sorted newest-first) of every event a specific player
-- appeared in a squad for, after applying the same filters as the main dashboard.
-- One element per distinct (event_id, squad_id) pair — a player in two squads
-- for the same event gets two rows (the drawer groups by squad via tabs).
...
GRANT EXECUTE ON FUNCTION public.get_player_squad_events_v2(integer, integer, timestamptz, timestamptz, integer[], text[], text[], integer, integer)
    TO anon, authenticated, service_role;
