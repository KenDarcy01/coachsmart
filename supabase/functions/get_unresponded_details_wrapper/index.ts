...
console.log('Event Email Reminder Wrapper Edge Function started!');
serve(async (req)=>{
  ...
  try {
    ...
    const { event_id: raw_event_id, p_role_grade: raw_role_grade, p_role_level: raw_role_level } = await req.json();
    const event_id = Number(raw_event_id);
    const p_role_grade = raw_role_grade ? Number(raw_role_grade) : null;
    const p_role_level = raw_role_level ? Number(raw_role_level) : null;
    ...
    console.log(`Filters - Grade: ${p_role_grade}, Level: ${p_role_level}`);
    // Call the PostgreSQL function, passing all three parameters
    const { data, error } = await supabaseClient.rpc('get_unresponded_events_v2', {
      event_id_param: event_id,
      p_role_grade: p_role_grade,
      p_role_level: p_role_level
    });
    if (error) {
    ...
    if (data && data.length > 0) {
      const emailsToSend = [];
      console.log(`Found ${data.length} unresponded members to email.`);
      for (const attendee of data){
        ...
        // Subject now mentions the specific member
        const subject = `${attendee.team_name || 'CoachSmart'}: Response needed for ${attendee.member_first_name} - ${attendee.event_title}`;
        const htmlBody = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #212529; color: #fff;">
            ...
            <div style="background-color: #2c313a; padding: 20px; border-radius: 8px;">
              ...
              <p style="font-size: 16px; color: #fff;">This is a friendly reminder to confirm attendance for <strong>${attendee.member_first_name} ${attendee.member_last_name}</strong> for the upcoming event:</p>
              ...
              <p style="font-size: 16px; color: #fff;">To help us finalise the team, please log onto the CoachSmart app and respond for ${attendee.member_first_name}.</p>
            </div>
      ...
      try {
        ...
        return new Response(JSON.stringify({
          message: `${data.length} reminders sent for this event`,
          data: result.data
        }), {
          status: 200,
          headers: {
      ...
      console.log('No unresponded attendees found for this event after applying filters.');
      return new Response(JSON.stringify({
