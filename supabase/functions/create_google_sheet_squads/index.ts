...
const handler = async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const [rows, adminRows, googleToken, userNameRows] = await Promise.all([
      sql`
        SELECT
    ...
    const firstName: string = userNameRows?.[0]?.first_name || 'there';
